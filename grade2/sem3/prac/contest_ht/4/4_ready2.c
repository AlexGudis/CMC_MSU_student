...
void
func_encode(uint8_t *out, uint8_t data_byte, uint8_t mask, uint8_t shift, char *ans, size_t *ans_ind)
{
    (*out) |= (data_byte & mask) >> shift;
...
b64u_encode(const uint8_t *data, size_t size)
{
    ...
    while (data_ind != size) {
        ...
        if (check_part == BYTE_ONE) {
            func_encode(&out, data_byte, MASK_BONE_UP, FIRST_SIX, ans, &ans_ind);
            out = (data_byte & MASK_BONE_DOWN) << SECOND_SIX;
            check_part = BYTE_TWO;
        } else if (check_part == BYTE_TWO) {
            func_encode(&out, data_byte, MASK_BTWO_UP, SECOND_SIX, ans, &ans_ind);
            out = (data_byte & MASK_BTWO_DOWN) << THIRD_SIX_LEFT;
            check_part = BYTE_THREE;
        } else if (check_part == BYTE_THREE) {
            func_encode(&out, data_byte, MASK_BTHREE_UP, THIRD_SIX_RIGHT, ans, &ans_ind);
...
void
func_decode(uint8_t *out, char str_byte, uint8_t mask, uint8_t shift, uint8_t *ans, size_t *ans_ind, int *check_smth,
            int flag)
{
    if (!flag) {
        (*out) |= (str_byte & mask) << shift;
        (*check_smth) = 1;
    } else {
        (*out) |= (str_byte & mask) >> shift;
        ans[*ans_ind] = *out;
...
b64u_decode(const char *str, uint8_t **p_data, size_t *p_size)
{
    ...
    while (str_ind != str_len) {
        ...
        if (check_part == S_ONE) {
            ...
            func_decode(&out, str_byte, MASK_S1, RIGHT_LEFT_SEC, ans, &ans_ind, &check_smth, 1);
            func_decode(&out, str_byte, MASK_S2, RIGHT_LEFT_SEC, ans, &ans_ind, &check_smth, 0);
            check_part = S_THREE;
        } else if (check_part == S_THREE) {
            func_decode(&out, str_byte, MASK_S3, RIGHT_S_THIRD, ans, &ans_ind, &check_smth, 1);
            func_decode(&out, str_byte, MASK_S4, LEFT_S_THIRD, ans, &ans_ind, &check_smth, 0);
            check_part = S_FOUR;
